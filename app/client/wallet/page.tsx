...
function WalletPageInner() {
  ...
  // Payment proof form — payMethod tracks which method the client actually
  // selected, so admin sees the true processor instead of it always
  // defaulting to Cash App.
  const [payMethod, setPayMethod] = useState<"cashapp" | "zelle">("cashapp");
    ...
    try {
      ...
      const res = await fetch("/api/wallet", {
        ...
        body: JSON.stringify({
          ...
          description: `${payMethod === "zelle" ? "Zelle" : "Cash App"} payment — $${amount}${payOrderNumber ? ` (Order #${payOrderNumber})` : ""}`,
        }),
      });
  ...
  const myCashAppSubmissions = transactions.filter((t) => t.purpose === "cashapp_topup" || t.purpose === "zelle_topup");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100">
              <h2 className="font-bold text-[#081A36]">Make a Payment</h2>
              <p className="text-xs text-slate-400 mt-0.5">Choose how you're paying, send it, then upload proof below</p>
            </div>

            {/* Method selector — the choice made here is what's submitted and
                shown to admin, so it must be explicit rather than inferred. */}
            <div className="px-5 pt-4 grid grid-cols-2 gap-3">
              <button
                type="button"
                onClick={() => setPayMethod("cashapp")}
                className={`flex items-center gap-2 justify-center rounded-xl border-2 px-3 py-2.5 text-sm font-bold transition-colors ${
                  payMethod === "cashapp"
                    ? "border-[#00D632] bg-[#00D632]/10 text-[#081A36]"
                    : "border-slate-200 text-slate-500 hover:border-slate-300"
                }`}
              >
                <span className="inline-flex items-center justify-center w-5 h-5 rounded-md bg-[#00D632] text-white text-xs font-black">$</span>
                Cash App
              </button>
              <button
                type="button"
                ...
                className={`flex items-center gap-2 justify-center rounded-xl border-2 px-3 py-2.5 text-sm font-bold transition-colors ${
                  payMethod === "zelle"
                    ? "border-[#6D1ED4] bg-[#6D1ED4]/10 text-[#081A36]"
                    : "border-slate-200 text-slate-500 hover:border-slate-300"
                }`}
              >
                <span className="inline-flex items-center justify-center w-5 h-5 rounded-md bg-[#6D1ED4] text-white text-xs font-black">Z</span>
                Zelle
              </button>
            </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs font-bold border border-slate-300 hover:border-[#00D632] hover:bg-[#00D632]/5 text-slate-700 px-3 py-2 rounded-lg"
                    >
