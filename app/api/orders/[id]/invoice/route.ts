...
<head>
<meta charset="UTF-8" />
<title>Invoice ${order.id} — FieldFlow</title>
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .header { background: linear-gradient(135deg, #1d4ed8, #3b82f6); color: white; padding: 40px; display: flex; justify-content: space-between; align-items: flex-start; }
  .brand { font-size: 24px; font-weight: 800; letter-spacing: -0.5px; }
  .brand span { opacity: 0.7; font-size: 14px; display: block; font-weight: 400; margin-top: 4px; }
  .invoice-num { text-align: right; }
  .invoice-num h2 { font-size: 20px; font-weight: 700; }
  .invoice-num p { opacity: 0.8; font-size: 13px; margin-top: 4px; }
  .body { padding: 40px; }
  ...
  th { text-align: left; padding: 10px 14px; background: #f1f5f9; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; color: #64748b; }
  td { padding: 12px 14px; border-bottom: 1px solid #f1f5f9; font-size: 14px; }
  .total-row { font-weight: 700; font-size: 15px; background: #eff6ff; }
...
<div class="page">
  <div class="header">
    <div class="brand">FieldFlow <span>Velocity REOs Field Services</span></div>
    <div class="invoice-num">
      <h2>INVOICE</h2>
      <p>#${order.id.toUpperCase()}</p>
      <p>Issued: ${fmt(order.createdAt)}</p>
    </div>
  </div>
  <div class="body">
    <div class="parties">
      <div class="party">
        ...
        <p>${client?.email ?? ""}</p>
        <p>${client?.phone ?? ""}</p>
        ${(client as { company?: string } | undefined)?.company ? `<p>${(client as { company: string }).company}</p>` : ""}
      </div>
      <div class="party">
        <h3>Field Agent</h3>
        <p class="name">${agent?.name ?? "Unassigned"}</p>
        ${agent ? `<p>${agent.email}</p><p>${agent.phone}</p>` : ""}
      </div>
    </div>
    <table>
      <thead><tr><th>Description</th><th>Service Type</th><th>Turnaround</th><th>Date</th><th style="text-align:right">Amount</th></tr></thead>
      <tbody>
        <tr>
          <td>${order.address}</td>
          <td style="text-transform:capitalize">${order.serviceType.replace(/_/g, " ")}</td>
          <td>${getTierLabel(order.turnaroundTier)}</td>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:16px;background:#f8fafc;border-radius:8px;margin-bottom:24px">
      ...
      <span class="${order.invoicePaid ? "paid" : "unpaid"}">${order.invoicePaid ? "✓ PAID" : "PENDING"}</span>
    </div>
    <div style="padding:16px;border:1px solid #e2e8f0;border-radius:8px">
      <p style="font-size:12px;color:#64748b;line-height:1.7">
        This invoice is generated by FieldFlow (Velocity REOs, Inc.). 
        ${order.photoExpiresAt ? `Photos for this order expire on <strong>${fmt(order.photoExpiresAt)}</strong>. ` : ""}
        Questions? Contact billing@fieldflow.com or call (941) 723-3200.
      </p>
    </div>
  </div>
  <div class="footer">
    <p>FieldFlow · fieldflow.com · (941) 723-3200</p>
    <p>Generated ${fmt(new Date().toISOString())}</p>
  </div>
</div>
</body></html>`;

  store.orders[store.orders.findIndex((o) => o.id === id)].invoicePaid =
    store.orders.find((o) => o.id === id)?.invoicePaid ?? false;
