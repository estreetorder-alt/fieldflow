    ...
    <div className="min-h-screen bg-[#FAF6EF] pt-20">
      ...
      <section className="bg-white text-[#2A2320] py-20 px-4">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-[#C2410C] font-bold text-sm uppercase tracking-wider mb-3">Get in Touch</p>
          <h1 className="text-4xl sm:text-5xl font-extrabold mb-5">Contact Us</h1>
          <p className="text-xl text-[#6B5D52]">Questions about orders, services, or agent applications? We're here to help.</p>
        </div>
      </section>
      ...
      <section className="py-16 px-4 bg-[#FAF6EF]">
        <div className="max-w-5xl mx-auto grid sm:grid-cols-3 gap-6 mb-12">
          {[
            ...
            <div key={c.title} className="bg-white border border-[#E7DBCB] rounded-2xl p-6 text-center hover:border-[#C2410C] transition-colors">
              <div className="w-12 h-12 bg-white text-[#C2410C] rounded-xl flex items-center justify-center mx-auto mb-4">{c.icon}</div>
              <h3 className="font-bold text-[#2A2320] mb-2">{c.title}</h3>
              {c.lines.map(l=><p key={l} className="text-[#6B5D52] text-sm">{l}</p>)}
              <p className="text-xs text-[#A99885] mt-2 italic">{c.note}</p>
            </div>
          ))}
        </div>
        ...
        <div className="max-w-3xl mx-auto">
          <h2 className="text-2xl font-bold text-[#2A2320] mb-6 flex items-center gap-2"><HelpCircle className="w-6 h-6 text-[#C2410C]"/>Common Questions</h2>
          <div className="space-y-4">
            {[
              ...
              <div key={q} className="bg-white border border-[#E7DBCB] rounded-xl p-5 hover:border-[#C2410C] transition-colors">
                <h3 className="font-semibold text-[#2A2320] mb-2">{q}</h3>
                <p className="text-[#8A7A6C] text-sm leading-relaxed">{a}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-3xl mx-auto grid sm:grid-cols-2 gap-6">
          <div className="bg-[#F3EBDD] rounded-2xl p-6">
            <Users className="w-8 h-8 text-[#C2410C] mb-3"/>
            <h3 className="font-bold text-[#2A2320] mb-2">Vendors</h3>
            <p className="text-[#6B5D52] text-sm mb-4">For order help, billing questions, or account issues:</p>
            <a href="mailto:info@snapect.com" className="inline-flex items-center gap-2 bg-[#C2410C] text-[#2A2320] font-bold px-4 py-2.5 rounded-xl text-sm hover:bg-[#EA580C] transition-colors">
              <Mail className="w-4 h-4"/>Email Vendor Support
            </a>
          </div>
          <div className="bg-[#F3EBDD] rounded-2xl p-6">
            <MessageSquare className="w-8 h-8 text-[#C2410C] mb-3"/>
            <h3 className="font-bold text-[#2A2320] mb-2">Field Agents</h3>
            <p className="text-[#6B5D52] text-sm mb-4">For application status, payment questions, or technical issues:</p>
            <a href="mailto:info@snapect.com" className="inline-flex items-center gap-2 bg-[#C2410C] text-[#2A2320] font-bold px-4 py-2.5 rounded-xl text-sm hover:bg-[#EA580C] transition-colors">
